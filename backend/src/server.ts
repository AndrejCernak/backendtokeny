...
const wss = new WebSocketServer({ server }); // path nepoužijem → funguje s ws://host:PORT
  ...
  ws.on("message", async (message: RawData) => {
    try {
      ...
      if (
        ...
        console.log(
          "[FW] type=%s from=%s(dev:%s) -> to=%s dev=%s callId=%s",
          type,
          currentUserId,
          currentDeviceId,
        ...
        sendToUser(data.targetId as string, forwarded, targetDeviceId);
        return;
      }
